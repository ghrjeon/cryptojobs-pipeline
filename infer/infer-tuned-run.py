...
def infer_job_function(df):
    ...
    for index, row in df.iterrows():
        ...
        response = aiClient.chat.completions.create(
            ...
            messages=[
                {"role": "user", "content": f"You are an expert HR professional specializing in job function classifications. \\\
                Job Title: {title} \n\nJob Function:"}
            ]
        )
...
def get_job_latest_data(table_name: str) -> pd.DataFrame:
    ...
    # Filter the dataframe to only include the latest date
    df = df[df['ingestion_date'] >= latest_date - pd.Timedelta(days=5)]
